...
public class GraphLogic {
    ...
    public void addConnection(Substation source, Substation target, double resistance) {
        // Verificar si las subestaciones existen en el grafo
        if (!graph.containsKey(source)) {
            System.out.println("Error: La subestación de origen no existe: " + source);
            return;
        }

        if (!graph.containsKey(target)) {
            System.out.println("Error: La subestación de destino no existe: " + target);
            return;
        }
